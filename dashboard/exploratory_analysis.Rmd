...
```{r}
exploratory_plot7 <- renderPlot(boxplot(MonthlyIncome~WorkLifeBalance,data = employee,xlab="PerformanceRating", ylab="MonthlyIncome"))
```
...
```{r}
exploratory_plot9 <- renderPlot(chart.Correlation(numeric, histogram=TRUE, pch=19))
```
...
exploratory_plot13 <- renderPlot(corrplot(correlation_data,method="circle"))
```
